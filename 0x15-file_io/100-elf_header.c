...
int main(int __attribute__((__unused__)) argc, char *argv[])
{
	...
	fcheklof(header->e_ident);
	printf("ELF Header:\n");
	magicletters(header->e_ident);
	thepreclass(header->e_ident);
	predatax(header->e_ident);
	provisiion(header->e_ident);
	theosibaq(header->e_ident);
	theabsencer(header->e_ident);
	proqtypei(header->e_type, header->e_ident);
	prdamentyw(header->e_entry, header->e_ident);
